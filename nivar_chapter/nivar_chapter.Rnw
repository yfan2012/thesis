...
what <- "figure"
figname <- "asms"
title <- "Characteristics of the JHU\\_Cniv\\_v1 assembly"
desc <- "{\\bf (A)} Cumulative lengths of the 50 longest sequences in our assembly and previous reference genome. {\\bf (B)} Ideogram of assembly. Sequence that is missing in the reference genome is shown along each non-mitochondrial contig, and the positions of telomere repeats are marked."
shortname <- "fig:asms"
where <- "[!hb]"
...
desc <- "Assembly statistics of JHU\\_Cniv\\_v1 and the reference genome for \\textit{C. nivariensis}."
shortname <- "tab:asmstats"
where <- "[!hb]"
...
what <- "table"
figname <- "telotable"
title <- "Contig and telomere lengths"
desc <- "Contig lengths and the number of times the forward and reverse telomere sequence appears in each"
shortname <- "tab:telotable"
where <- "[!ht]"
...
what <- "figure"
figname <- "telopos"
title <- "Telomere positions reference based scaffolds"
desc <- "Histogram of telomere repeat positions in our assembly, and in scaffolds produced by RagTag and MeDuSa. When MeDuSa is used with a database including the reference genomes of \textit{C. nivariensis}, \textit{C. glabrata}, C. bracarensis, and N. delphensis, telomeres are placed in the middle of contigs. The same result is produced when only the \textit{C. glabrata} genome is used for scaffolding with MeDuSa, and MeDuSa fails to run when only the \textit{C. nivariensis} reference is used. When the \textit{C. nivariensis} reference genome is used for scaffolding with RagTag, no changes are made. When the more contiguous \textit{C. glabrata} genome is used with RagTag, telomere sequences are again placed in the middle of sequences, suggesting a scaffolding error. "
shortname <- "fig:telopos"
where <- "[!ht]"
...
what <- "figure"
figname <- "mummer"
title <- "Whole genome alignment of JHU\\_Cniv\\_v1 and the \textit{C. nivariensis} reference genome"
desc <- "Whole genome alignment of the current reference genome (y axis) compared to our new assembly (x axis). Alignments match with no notable structural variants, and very little missing or duplicated sequence."
shortname <- "fig:mummer"
where <- "[!ht]"
...
<<results = 'asis', eval = TRUE>>=
what <- "table"
figname <- "genecounttable"
title <- "Contributions from each annotation software"
desc <- "Number of genes and exons added by each software"
shortname <- "tab:telotable"
where <- "[!ht]"
...
<<results = 'asis', eval = TRUE>>=
what <- "table"
figname <- "refgenestable"
title <- "Gene and exon counts of JHU\\_Cniv\\_v1 and related yeasts"
desc <- "Gene and exon counts of our annotation and currently available reference annotations"
shortname <- "tab:telotable"
where <- "[!ht]"
...
what <- "figure"
figname <- "mito"
title <- "Alignment of JHU\\_Cniv\\_v1 mitochrondrial contig and the \textit{C. nivariensis} mitochondrial genome"
desc <- "Alignment of our 32Kb circular contig (x axis) with the completed mitochondrial genome of the \textit{C. nivariensis} reference genome (y axis). The final 3662bp of this contig appears twice in the reference genome."
shortname <- "fig:mito"
where <- "[!ht]"
...
desc <- "Histogram of coverage per base in our assembly by filtered (>3kb) ONT reads and trimmed Illumina reads."
shortname <- "fig:covhist"
where <- "[!ht]"
